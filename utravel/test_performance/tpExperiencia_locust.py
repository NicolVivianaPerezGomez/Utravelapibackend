...
#PRUEBAS DE DESEMEPEÑO (TIEMPO DE RESPUESTA) CON LOCUST TIPO DE EXPERIENCIAS
...
class TpExperiencia(HttpUser): #HttpUser usuario virtual con peticion http

    wait_time = between(1,3) #tiempo de espera entre peticiones

    experiencia_creada_id = None 
    ...
    #MÉTODO PARA PROBAR LISTAR EXPERIENCIAS

    @task(3) #decorador que indica que esta accion la eejcuta un usuario con un peso de 1 
    def listar(self):
        self.client.get("/tipo_exp/", headers=self.headers) #client hace referencia al cliente
    ...
    #MÉTODO PARA CREAR EXPERIENCIAS 

    @task(1)
    def crear_experiencias(self):

        tipexp_descripcion= f"Experiencia {self.nombre_valido(5)}"#guarando un ejemplo de registro con un nombre con método de random string

        response = self.client.post("/tipo_exp/", json={
            "tipexp_descripcion": tipexp_descripcion
        }, headers=self.headers)

        if response.status_code in (200, 201):

            try:
                self.experiencia_creada_id = response.json.get("tipexp_id")
            except:
    ...
    def buscar_por_id(self):
        if self.experiencia_creada_id:
            self.client.get(f"/tipo_exp/{self.experiencia_creada_id}/", headers=self.headers)

    #MÉTODO PARA PROBAR ACTUALIZAR
    @task(1)
    def actualizar_experiencia(self):

        new_descripcion = f"Experiencia {self.nombre_valido(5)}" #nuevo nombre random

        if self.experiencia_creada_id:
            self.client.put(f"/tipo_exp/{self.experiencia_creada_id}/", json={
                "tipexp_descripcion": new_descripcion
            }, headers=self.headers)
...
ejecutar locust -f tpExperiencia_locust.py --host=http://127.0.0.1:8000
